...
def get_domain_content_info(content):
    ...
    for property_info in content.split('css-1qp9106" data-testid="')[1:]:
        current_property = pd.Series()
        current_property['Price'], property_info = distill_from_pattern(property_info,
                                                                        '"listing-card-price" class="css-mgq8yx">')
        current_property['Url'], property_info = distill_from_pattern(property_info,
                                                                      '<a href="',
                                                                      '"')
        current_property['Name'], property_info = distill_from_pattern(property_info,
                                                                       'itemProp="streetAddress" class="css-iqrvhs">')
        current_property['Name'] += ' (D)' + current_property['Url'][-4:]
        current_property['Type'], property_info = distill_from_pattern(property_info,
                                                                       'class="css-693528">')
...
def get_candidate_domain_properties():
    ...
    while True:
        ...
        if re.search('No exact matches', content):  # If this page contains no properties.
            break
        property_info = property_info.append(get_domain_content_info(content), sort=False, ignore_index=True)
        page += 1
...
def get_domain_details(url):
    ...
    start_point = re.search('listingSummary":{', content).end()
    slice_str = content[start_point:]
    detail_info['Bedroom_num'] = slice_str[re.search('beds":', slice_str).end()]
    detail_info['Bathroom_num'] = slice_str[re.search('baths":', slice_str).end()]
    try:
        detail_info['Parking_num'] = slice_str[re.search('parking":', slice_str).end()]
    except AttributeError:
        print("No idea what is happening here")
    return detail_info


def initiate_domain_properties(property_info):
...
def get_realestate_properties():
    ...
    east = EAST
    page_size = 150
    content = requests.get(f"https://services.realestate.com.au/services/listings/search?query="
        ...
        .content.decode('UTF-8')  # At most 150 properties are include which seems to be enough in this case.
    property_info = pd.DataFrame()
    current_property = pd.Series()
    ...
    while re.search('"lister":', left_content):
        ...
        try:
            property_content = left_content[:re.search('"lister":', left_content).end()]
        except AttributeError:
        ...
        if re.search('dateAvailable":{"date":"', property_content):
            current_property['Available'] = slice_line(property_content,
                                                       re.search('dateAvailable":{"date":"',
    ...
    if len(property_info) == page_size:
        print("Warning: number of properties found may larger than the upper bound.")
...
def initiate_property_data(file, domain_property_info, realestate_property_info):
    ...
    property_data = domain_property_data.append(realestate_property_info, ignore_index=True,
        ...
        for col in now_data:
            try:
                if previous_data.loc[name, col] != now_data.loc[name, col]:
                    diff_value[col] = [previous_data.loc[name, col], now_data.loc[name, col]]
            except ValueError:  # Duplicate index.
                if not previous_data.loc[name, col].equals(now_data.loc[name, col]):
                    diff_value[col] = [previous_data.loc[name, col].iloc[0], now_data.loc[name, col].iloc[0]]
    ...
    for name in diff_dict:
        if name in preferred_names:
            ...
            for value in diff_dict[name]:
                print(f"{value} from '{diff_dict[name][value][0]}' to '{diff_dict[name][value][1]}'. ", end='')
            print()
        else:
            continue
    return
...
def update_property_data(file, previous_data, domain_property_info, realestate_property_info, log_file):
    common_col = ['Price', 'Type', 'Url', 'Source']
    now_data = domain_property_info[common_col + ['Name']].append(realestate_property_info[common_col + ['Name']],
                                                                  ignore_index=True,
                                                                  sort=False).sort_values('Name')
    ...
    if previous_data[common_col].sort_values('Url').equals(now_data.sort_values('Url')):
        ...
        if not previous_data.loc[same_names, common_col].sort_values('Url').equals(
                now_data.loc[same_names].sort_values('Url')):
            if_no_diff = previous_data.loc[same_names, common_col].sort_values('Url').eq(
                now_data.loc[same_names].sort_values('Url')).all(axis=1)
            name_of_diff = if_no_diff[~if_no_diff].index
            diff_dict = get_diff_info(name_of_diff, previous_data, now_data)
            previous_data.loc[name_of_diff, common_col] = now_data.loc[name_of_diff, common_col]
        if len(new_names) > 0:
            add_data = initiate_property_data(None,
        ...
        with open(log_file, 'a+') as f:
            ...
            if len(passed_names) > 0:
                ...
                print(f"No longer available preferred properties: {passed_names & preferred_names}")
        return add_data
...
def monitor_properties(file, log_file):
    ...
    try:
        ...
        return update_property_data(file, previous_data, domain_property_info, realestate_property_info, log_file)
    except FileNotFoundError:
        present = time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime())
        with open(log_file, 'w+') as f:
            f.write(f"Start monitoring properties in {file} at: {present} (UTC)\n")
        print('Property monitor initiating...')
        return initiate_property_data(file, domain_property_info, realestate_property_info)
